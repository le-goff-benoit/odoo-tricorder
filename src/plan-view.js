...
export function planViews({ get, observations, esc, badge, render, board }) {
  ...
  function taskCard(t) {
    if (t.kind === 'intention') return `<article class="plan-task-row intention" data-plan-task="${esc(t.id)}" data-kind="intention"><button class="task-card" data-view="intentions" data-intention="${esc(t.intentionId)}"><div class="task-description"><div class="task-topline"><span class="task-id">${esc(t.intentionId)}</span>${badge('pending', t.presentation.label)}</div><h3>${esc(t.title)}</h3><p>Demande · ${esc(t.reason)}</p><small>Consulter la demande et ses décisions · pas encore exécutable</small></div></button></article>`;
    const p = t.presentation || taskState(t);
    return `<article class="plan-task-row ${t.kind || ''}" data-plan-task="${esc(t.id)}"><button class="task-card" data-task="${esc(t.id)}" aria-haspopup="dialog"><div class="task-marker ${esc(t.status)}">${p.received ? '✓' : '·'}</div><div class="task-description"><div class="task-topline"><span class="task-id">${esc(t.kind === 'orchestration' ? 'Principal' : t.id)}</span>${badge(p.received ? 'received' : t.status, t.kind === 'orchestration' ? 'Orchestration' : p.label)}${t.risk === 'high' ? '<span class="risk">Sensible</span>' : ''}</div><h3>${esc(t.title)}</h3><p>${esc(t.kind === 'orchestration' ? t.stage + (t.model ? ' · ' + t.model : ' · modèle principal non renseigné') : p.proof || t.reason || p.label)}</p>${t.depends_on?.length ? `<div class="dependencies">Après ${esc(t.depends_on.join(', '))}</div>` : ''}<small>${t.kind === 'orchestration' ? esc(t.owners.join(' · ') || 'Responsable non renseigné') : (t.acceptance?.length || 0) + ' critères · ouvrir les détails'}</small></div></button>${activity(t)}</article>`;
  }
  ...
  function plan() {
    ...
    return `<div class="page plan-page ${mode() === 'kanban' ? 'release-kanban' : ''} ${d.selectedRelease ? '' : 'no-release'}" data-context="${esc(context())}">${!d.selectedRelease ? '<h2>Aucune release sélectionnée</h2><p>La préparation appartient au projet. Les intentions et le plan seront rattachés à la release lors de sa création.</p>' : ''}<div class="section-title"><div><h2>Plan de release</h2>${release ? `<span class="release-state">${badge(release.status, release.status === 'close' ? 'Close' : release.status === 'ouverte' ? 'Ouverte' : release.status)}</span>` : ''}</div><span class="release-counts board-count">${counts}</span></div>${(d.warnings || []).map(w => `<div class="note warning">${esc(w)}</div>`).join('')}<div class="plan-toolbar"><div role="group" aria-label="Présentation du plan">${modes.map(([id, label]) => `<button class="secondary" data-plan-mode="${id}" aria-pressed="${mode() === id}">${label}</button>`).join('')}</div><div role="group" aria-label="Filtrer le plan" class="board-filters">${filters.map(([id, label]) => `<button class="secondary" data-plan-filter="${id}" aria-pressed="${saved.filter === id}">${label}</button>`).join('')}</div><label class="board-owner-filter">Responsable <select id="board-owner" aria-label="Filtrer par responsable"><option value="">Tous</option>${owners.map(o => `<option ${saved.owner === o ? 'selected' : ''}>${esc(o)}</option>`).join('')}</select></label>${d.selectedRelease ? `<span class="release-links"><button class="text-button" data-document="changelog/${esc(d.selectedRelease)}/README.md">Lire le bilan de release</button></span>` : ''}</div>${body}${!tasks.length ? '<p class="note">Aucune tâche structurée. L’orchestrateur prépare le plan à partir des intentions.</p>' : ''}</div>`;
  }
  function intentions() {
